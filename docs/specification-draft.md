...
- APIキー・OAuthトークン等はワークスペースJSONや平文設定へ直接格納せず、OSで保護する別ストアに保存し、設定側には参照を持たせる。具体方式は実装前に決定する。
...
| 1. 前提確認・PoC | Windows通知と配布方式、初期メール・カレンダーProvider、Providerキャッシュを確認 | 通知PoC合格、初期連携先と方式の確定 |
...
| --- | --- | --- | --- |
| 高 | Windows通知の実装・配布方式 | ネイティブ連携と必要なパッケージ構成をPoCで比較 | 段階1 |
