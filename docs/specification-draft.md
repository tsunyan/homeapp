...
本書は添付された構想文を仕様へ整理し、WinUI 3で実装した操作と技術選定を反映した草案である。自由配置、リサイズ、複数ボード、ローカル保存、時計、メモ、手入力のカスタムJSONに加え、Gmail・Googleカレンダーの読み取り、公開RSS・Atomの取得、Windows通知の表示を実装している。

現在のGoogle接続は1アカウントの受信トレイ最新20件とメインカレンダーに対応する。RSSはウィジェットごとに1配信元の最新30件を表示する。これらは表示時と手動更新時に取得し、カレンダーは日付変更時にも取得する。Windows通知はアプリ登録とアクセス許可のもとで最新50件を15秒ごとに取得する。以下に記す共通の定期更新・キャッシュ・REST/JSONのURL取得や配布パッケージは今後の実装範囲であり、MVP全体の完了を示すものではない。接続手順は[Google接続ガイド](google-connection.md)と[Windows通知ガイド](windows-notifications.md)を参照する。
...
| 画面 | 特定解像度を前提にしない。小さなウィンドウから4Kまで、スクロールとズームで同じ保存配置を利用可能にする |
...
| F-03 | Calendar | 選択したカレンダーの予定一覧 | 初期ProviderはGoogle Calendar。閲覧中心 |
| F-04 | Mail | 未読件数と最近のメール一覧 | 初期ProviderはGmail。送信・返信は含めない |
...
初回起動時は操作感を確認できる2つのボードを表示する。メールと予定はGoogle接続、RSSは配信元登録、Windows通知はアプリ登録やアクセス許可の状態に応じた案内を表示する。メモとカスタムJSONには編集可能な入力例を用意する。空のボードでも追加ガイドと「ウィジェットを追加」を表示する。
...
- 位置とサイズは、ボード左上を原点とする解像度非依存の座標で保持する。列数や基準解像度には固定しない。
- ウィジェットは空白や重なりを含めて任意の位置へ置ける。グリッドへのスナップは利用者が切り替えられる補助機能とする。
- ウィンドウのサイズ変更、表示倍率の変更、別モニターへの移動では保存座標を変更しない。見える範囲はスクロールし、35〜200%のズームと「全体を表示」を提供する。
- 「整列」は利用者が明示的に実行する。現在の表示幅に合わせて左上から折り返すが、各ウィジェットのサイズと順序は保持する。
- 初回だけ現在の表示幅から1〜8列を選び、サンプルウィジェットを配置する。4Kでは余白を有効に使い、狭い画面では1列から始める。
- 各Widget定義は最小幅・最小高さ・初期サイズを持つ。プロトタイプの共通最小値は幅240、高さ180とする。
- 内容が収まらないウィジェットは内部スクロールし、ボード全体は縦横にスクロールできる。
...
- 初期ProviderはGoogle Calendarとし、現在はメインカレンダーの選択日の予定を読み取る。Outlook等は追加Providerの候補とする。
...
- 初期ProviderはGmailとし、現在はGoogleカレンダーと同じ1アカウントの受信トレイ最新20件を読み取る。Outlook等は追加Providerの候補とする。
...
Windowsネイティブのコントロールと操作感を優先し、**C# / .NET 10 + WinUI 3 + Windows App SDK** を採用する。UIはXAML、状態・保存・レイアウト計算はWinUIに依存しない`HomeApp.Core`へ分離する。x64のunpackaged・self-contained構成で動作し、Windows通知に必要なidentityはローカル開発用スクリプトで登録する。署名・インストーラーを含む配布形式は引き続き検証対象とする。

```mermaid
flowchart TB
  User[利用者] --> UI[WinUI 3 / XAML]
  UI --> Canvas[自由配置キャンバス]
  UI --> Widgets[WidgetFrame 群]
  Canvas --> Layout[HomeApp.Core レイアウト計算]
  Widgets <-->|型付きモデル| Core[HomeApp.Core]
  Core --> Scheduler[更新管理]
  Scheduler --> Providers[Provider 群]
  Providers --> Services[メール・予定・RSS・REST]
  Providers --> Native[Windows通知連携]
  Core --> Store[(ユーザー別JSON: 設定・配置)]
  Providers --> Cache[(将来のProviderキャッシュ)]
  Core --> Secrets[OSで保護する秘密情報ストア]
```

- UIは秘密情報を保持せず、必要な操作を型付きのサービス経由で要求する。
- 外部通信、認証、保存、OSアクセスはC#の共通サービスへ集約する。
- まず組込みWidget・Providerの登録方式を用意し、外部コードを動的に読み込む公開SDKは後から設計する。
- 配置は`Canvas`上の解像度非依存座標で扱い、`ScrollViewer`のパン・スクロール・ズームと組み合わせる。画面サイズ変更は保存済み座標を書き換えない。
...
開発環境で動くだけでは合格としない。Windows App SDKからのWinRT呼出し、パッケージIDの付与、MSIXとunpackagedの差を比較し、必要な方式をこの段階で確定する。
...
| エンティティ | 主な項目 |
| --- | --- |
| Board | id、名前、ズーム、初期配置済みフラグ。複数件を保持 |
| WidgetInstance | id、boardId、type、title、providerConnectionId、config、設定版 |
...
- APIキー・OAuthトークン等はワークスペースJSONや平文設定へ直接格納しない。実装済みのGoogle接続では、`EncryptedGoogleStore`が`DataProtectionProvider("LOCAL=user")`でOAuthトークンとクライアント設定を現在のWindowsユーザー向けに暗号化し、`workspace.json`とは別の`%LOCALAPPDATA%\HomeApp\GoogleAuth`に保存する。将来のProvider接続モデルでは秘密情報への参照を持たせる。
...
- 保存スキーマを版管理する。書込みは一時ファイルから原子的に置換し、直前のバックアップを1世代保持する。読込み失敗時は破損ファイルを回復用コピーとして保全してからバックアップを試す。
...
- Windowsのcapabilityとアプリ権限は必要な機能に限定する。Providerの引数・接続先・リダイレクトを検証し、埋め込みWebコンテンツからネイティブ操作を直接呼べる状態にしない。
...
| A-03 | F-01 | ウィンドウサイズ、Windowsの表示倍率、モニターを変更しても保存座標は変わらず、スクロール・ズームで全ウィジェットへ到達できる |
...
| 段階 | 内容 | 完了判断 |
| --- | --- | --- |
| 0. 操作プロトタイプ | WinUI 3、Widget登録、自由配置、ズーム、複数ボード、JSON保存、時計、メモ、サンプル表示 | 追加・移動・リサイズ・取消・保存・再起動復元を実機確認 |
| 1. 前提確認・PoC | 実装済みのGmail・Google Calendar・UserNotificationListenerを前提に、通知の配布構成・受入条件とProviderキャッシュを検証 | 配布構成の確定、配布環境での通知PoC合格、キャッシュ方針の確定 |
...
操作プロトタイプと、Google・RSS・Windows通知の実データ表示は実装済みである。共通の更新管理、REST/JSONのURL取得、配布構成と受入条件の確認は残っている。工数・日程は、残る連携・配布上の制約を確認してから見積もる。

## 15. 要決定事項

| 優先度 | 項目 | 草案の置き方 | 決定が必要な時点 |
| --- | --- | --- | --- |
| 高 | Windows通知の配布構成・検証 | 実装済みのUserNotificationListenerを前提に、署名・パッケージ・インストーラーの構成を決定し、配布環境で権限・復帰動作を検証 | 段階1 |
...
| 中 | 対応画面・性能基準 | 解像度は固定しない。小ウィンドウ、4K、表示倍率100〜200%の評価条件とCPU上限を確定 | UI・性能評価前 |
...
- UI技術はWinUI 3とする。
- 保存レイアウトは固定列ではなく解像度非依存座標とし、任意配置と重なりを許可する。
- グリッドスナップは任意、ウィンドウ変更時の再配置は行わず、「整列」だけを明示操作にする。
- 初期メール連携先はGmail、初期カレンダー連携先はGoogle Calendarとし、同じ1アカウントから読み取る。
- 未接続のウィジェットでは接続・配信元登録・アクセス許可の案内を表示する。
